...
class Point(object):
        ...
        def get_distance_from(self,p2):
            import math
            distance = math.sqrt((p2.y - self.y)**2 + (p2.x - self.x)**2)
            return distance
        ...
        def get_distance_traveled(self):
            distance = (self.x - self.initial_x) + (self.y - self.initial_y)
            return distance

        def closer_to(self, p2, p3):
            import math
            distance1 = math.sqrt((p2.y - self.y)**2 + (p2.x - self.x)**2)
            distance2 = math.sqrt((p3.y - self.y)**2 + (p3.y - self.y)**2)
